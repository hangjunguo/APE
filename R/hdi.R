...
#' @title Adaptive Projection Estimation
#' @description Adaptive Projection Estimation
#' @param X the design matrix
#' @param y the response
#' @param S the prescreened set
#' @param alpha the significance level alpha, default is 0.05
#' @return An APE object is returned, including confidence intervals for regression coefficients. 
#' @examples
#' \dontrun{
#' dat <- simdata(n, p, rho, truebeta)
#' X <- dat$X
...
#' ape <- AP(X, y, S)
#' CI <- ape$CI; len_CI <- ape$len_CI
#' }
#' @export
AP <- function(X, y, S, alpha=0.05){
